...
public class ProfileEvaluator {
	...
	private static final String SQL_AVG_AI = "SELECT COUNT(s.id) as total, AVG(CAST(a.ai_generated_probability AS FLOAT)) as avg_ai "
			...
			+ "JOIN ai_analysis a ON s.id = a.submission_id " + "WHERE t.username = ?";

	private static final String SQL_TOP_ALGO = "SELECT TOP 3 a.algorithm, COUNT(*) AS usage_count "
			+ "FROM submission s " + "JOIN target_account t ON s.account_id = t.id "
			+ "JOIN ai_analysis a ON s.id = a.submission_id "
			+ "WHERE t.username = ? AND a.algorithm IS NOT NULL AND a.algorithm NOT IN (N'N/A', N'Error') "
			+ "GROUP BY a.algorithm ORDER BY usage_count DESC";

	// PUBLIC: Fetch user stats for UI
	public static UserStats fetchUserStats(String username) throws Exception {
		try (Connection conn = LastProjectJava.getConnection()) {
			...
			int totalSubs = aiStats.totalSubmissions;
			int analyzed = getAnalyzedCount(conn, username);

			return new UserStats(totalSubs, analyzed, aiStats.avgAi, algorithms);
	...
	private static int getAnalyzedCount(Connection conn, String username) throws Exception {
		String sql = "SELECT COUNT(*) FROM submission s " + "JOIN target_account t ON s.account_id = t.id "
				+ "INNER JOIN ai_analysis ai ON s.id = ai.submission_id " + "WHERE t.username = ?";
	...
	private static String buildReportString(AiStats ai, List<String> algos) {
		...
		if (ai.avgAi > AI_HIGH_THRESHOLD)
			sb.append("Lạm dụng cao.\n");
		else if (ai.avgAi > AI_MEDIUM_THRESHOLD)
			sb.append("Có tham khảo AI.\n");
		else
			sb.append("Tự lực tốt.\n");
	...
	// PUBLIC: User Stats class
	public static class UserStats {
		public int totalSubmissions;
		public int analyzedCount;
		public double avgAi;
		public List<String> topAlgorithms;

		public UserStats(int total, int analyzed, double avg, List<String> algos) {
			this.totalSubmissions = total;
			this.analyzedCount = analyzed;
			this.avgAi = avg;
			this.topAlgorithms = algos;
		}
	}
